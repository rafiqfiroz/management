  ...
  <div>
    <div class="row">
      ...
      <div class="col-md-8">
        ...
        <div class="card-body">
            <h3 style="text-align:center;text-decoration:underline">Student Manage</h3>
            <router-link to="add_student" style="float:right" class="btn btn-success btn-sm">Add Student</router-link>
          <table class="table table-striped" id="datatable">
            ...
            <tbody>
              <tr v-for="(AllCategories,index) in AllSize" :key="AllCategories.id">
                 <!-- <td><input type="checkbox" :value="AllCategories.id" v-model="selected"></td> -->
                <td>{{index+1}}</td>
                <td>{{AllCategories.name}}</td>
                <td>{{AllCategories.department.name}}</td>
                <td>{{AllCategories.batch_name}}</td>
                ...
                <td>
                    ...
                    <router-link :to="`edit_student/${AllCategories.id}`" class="btn btn-success btn-sm"><i class="fa fa-edit"></i></router-link>
                </td>
              </tr>
...
export default {
    ...
    methods: {
         ...
         DeleteItem(Id){
          axios.get('/api/auth/student-delete/'+Id)
          .then((result) => {
